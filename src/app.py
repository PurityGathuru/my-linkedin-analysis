...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            ...
            dbc.Card([
                dbc.CardBody([
                    dbc.CardLink("Purity Gathuru", target="_blank",
                                 href="https://www.linkedin.com/in/purity-gathuru-341a71135/"
                    )
                ])
            ]),
        ], width=2),
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.A("https://github.com/PurityGathuru/my-linkedin-analysis", href='GitHub', target="_blank")
                ])
            ]),
        ], width=12),
    ],className='mb-2'),
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dcc.Graph(id='wordcloud_1', figure={}, config={'displayModeBar': False}),
                ])
            ]),
        ], width=4),
        dbc.Col([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    dcc.Graph(id='wordcloud_2', figure={}, config={'displayModeBar': False}),
                ])
            ]),
        ], width=4),
    ],className='mb-2'),
...
# Jobs Applied Word Cloud ************************************************************
@app.callback(
    Output('wordcloud_1','figure'),
    Input('my-date-picker-start','date'),
    Input('my-date-picker-end','date'),
)
def update_pie(start_date, end_date):
    dff = df_job.copy()
    dff = dff.Company[(dff['Application Date']>=start_date) & (dff['Application Date']<=end_date)].astype(str)
    ...
    fig_wordcloud = px.imshow(my_wordcloud, template='ggplot2',
                              title="Jobs Applications by Company")
...
def update_pie(start_date, end_date):
    ...
    msg_sent = len(dff[dff['FROM']=='Roy Khasiani Murwa'])
    msg_rcvd = len(dff[dff['FROM']!='Roy Khasiani Murwa'])
    fig_pie = px.pie(names=['Sent','Received'], values=[msg_sent, msg_rcvd],
                     template='ggplot2', title="Messages Sent & Received"
                     )
    fig_pie.update_layout(margin=dict(l=20, r=20, t=30, b=20))
    fig_pie.update_traces(marker_colors=['yellow','orange'])
